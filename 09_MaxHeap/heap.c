...
void heapify_up(Heap* heap, int idx) {
    ...
    if (current->priority > parent->priority) {  // Changed comparison to >
...
void heapify_down(Heap* heap, int idx) {
    ...
    int largest = idx;  // Changed variable name from smallest to largest

    if (left_child_idx < heap->size &&
        heap->nodes[left_child_idx].priority > heap->nodes[largest].priority) {  // Changed comparison to >
        largest = left_child_idx;
    }

    if (right_child_idx < heap->size &&
        heap->nodes[right_child_idx].priority > heap->nodes[largest].priority) {  // Changed comparison to >
